...
Before I get into explaining what you see on the front page of my site, I thought I'd drop a diagram as a quick overview so here it is:

[[/p3_diagram.png]]

What you see on my front page is actually mean to be a full dive through the workings of a modern machine, starting at the program level and going all the way down to the hardware logic level. This is then run through an [[https:github.com/danbrodsky/webRTL][FPGA simulator]] compiled to WASM so that it can run in the browser.
